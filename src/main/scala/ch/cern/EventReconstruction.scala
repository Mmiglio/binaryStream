    ...
    stream.foreachRDD(rdd => {
      if(!rdd.isEmpty()){
        ...
        if(!selectedOrbits.take(1).isEmpty){
          ...
          // Select hits with ORBIT_CNT in the table "selectedOrbits"
          val events = convertedDF
            .join(selectedOrbits,"ORBIT_CNT")

          // Collapse dataframe -> needed to convert the dataframe into a json
          ...
          events2kafka.foreachPartition(partition => {
